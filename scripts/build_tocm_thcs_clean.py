...
BASE_MASTER = r'D:\UNIGO\Hệ thống mẫu văn bản\09.07.26. Kế hoạch tổ chuyên môn (THCS).docx'
TARGET_MASTER = os.path.join(DIR_TOCM, 'Kế hoạch tổ chuyên môn (THCS).docx')

if not os.path.exists(BASE_MASTER):
    BASE_MASTER = TARGET_MASTER

def clean_xml_element(elem):
    """Strips duplicate bookmarks and invalid r:id attributes from element."""
    # Remove bookmarkStart and bookmarkEnd
    for bm in elem.xpath('.//w:bookmarkStart | .//w:bookmarkEnd'):
        bm.getparent().remove(bm)
    # Remove r:id attributes if no matching rel
...
def set_table_borders(table):
    """Sets explicit full black grid borders XML."""
...
def append_element_before_sectpr(doc, elem):
    """Appends an element to doc body BEFORE the last w:sectPr so OpenXML structure remains 100% valid."""
...
def append_p(doc, text, bold=False, italic=False, align=WD_ALIGN_PARAGRAPH.LEFT, font_size=13):
    ...
    # Ensure paragraph is before sectPr
    p_elem = p._element
...
def append_table_copy(doc, src_table):
    ...
    append_element_before_sectpr(doc, tbl_elem)

def main():
    print(f"Reading base template from {BASE_MASTER}...")
    doc_base = docx.Document(BASE_MASTER)
    ...
    for t_i in range(min(9, len(doc_base.tables))):
        append_table_copy(doc_out, doc_base.tables[t_i])
        append_p(doc_out, "")
    ...
    try:
        doc_out.save(TARGET_MASTER)
        print(f"\nSuccessfully built and saved clean master document: {TARGET_MASTER}")
    except PermissionError:
        fallback = os.path.join(DIR_TOCM, 'Kế hoạch tổ chuyên môn (THCS) - Mới.docx')
        doc_out.save(fallback)
        print(f"\nPermission error saving {TARGET_MASTER}, saved fallback: {fallback}")
